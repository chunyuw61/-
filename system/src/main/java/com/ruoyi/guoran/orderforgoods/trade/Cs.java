package com.ruoyi.guoran.orderforgoods.trade;
 ...
 */
public class Cs {
    private static Log log = LogFactory.getLog(Cs.class);
    ...
    public static void init() {
        Configs.init("system/src/zfbinfo.properties");
    ...
    public static String test_trade(String shopName, String sum, String quantity, String orderNumber) {
        Cs cs = new Cs();
        cs.init();
        return test_trade_precreate(shopName, sum, quantity, orderNumber);
    }
...
/*        Cs cs = new Cs();
        ...
        // 测试当面付2.0生成支付二维码
       /* cs.test_trade_precreate("巴黎世家", "1", "18989", "12345678902");*/
        /*        main.test_trade_query();*/
/*        byte[] imageBytes = imageToBytes("order/src/main/java/com/ruoyi/orderforgoods/pic/qr-202401041543327345.png");
        System.out.println(Arrays.toString(imageBytes));*/

        // 设置要删除的文件路径
       /* String filePath = "order/src/main/java/com/ruoyi/orderforgoods/pic/qr-1234567890.png";*/
    ...
    // 测试当面付2.0查询订单
    public static String test_trade_query(String numberPart) {
        // (必填) 商户订单号，通过此商户订单号查询当面付的交易状态
        String outTradeNo = numberPart;
    ...
    // 测试当面付2.0生成支付二维码
    public static String test_trade_precreate(String shopName, String sum, String quantity, String orderNumber) {
        ...
        // 设置 subject 为店名加上 "当面付扫码消费"
        String subject = shopName + "当面付扫码消费";
        // 设置 totalAmount 为总金额
        String totalAmount = quantity;
        String undiscountableAmount = "0";
        String sellerId = "";
        // 设置 body 为 "购买商品" 加上商品数量和总金额
        String body = "购买商品" + sum + "件共" + quantity + "元";
        // 设置 operatorId 为 "test_operator_id"
        String operatorId = "test_operator_id";
        // 设置 storeId 为 "test_store_id"
        String storeId = "test_store_id";
        // 创建一个新的 ExtendParams 对象
        ExtendParams extendParams = new ExtendParams();
        // 设置 extendParams 的 sysServiceProviderId 为 "2088721023728055"
        extendParams.setSysServiceProviderId("2088721023728055");
        // 设置 timeoutExpress 为 "120m"
        String timeoutExpress = "30m";
        // 创建一个新的 goodsDetailList
        List<GoodsDetail> goodsDetailList = new ArrayList<GoodsDetail>();
        // 创建一个新的 GoodsDetail 对象并添加到 goodsDetailList 中
        GoodsDetail goods1 = GoodsDetail.newInstance("goods_id001", "xxx小面包", 1000, 1);
        goodsDetailList.add(goods1);
        // 创建另一个新的 GoodsDetail 对象并添加到 goodsDetailList 中
        GoodsDetail goods2 = GoodsDetail.newInstance("goods_id002", "xxx牙刷", 500, 2);
        goodsDetailList.add(goods2);
        // 创建一个新的 AlipayTradePrecreateRequestBuilder 对象并设置各种参数
        ...
        // 调用 tradeService 的 tradePrecreate 方法来创建预下单请求
        AlipayF2FPrecreateResult result = tradeService.tradePrecreate(builder);
        // 根据预下单请求的结果来执行不同的操作
        switch (result.getTradeStatus()) {
            case SUCCESS:
                // 如果预下单成功，打印出成功信息，并获取响应
                log.info("支付宝预下单成功: )");
                AlipayTradePrecreateResponse response = result.getResponse();
                dumpResponse(response);
                // 设置 filePath 为二维码图片的路径
                filePath = String.format("system/src/main/java/com/ruoyi/guoran/orderforgoods/pic/qr-%s.png",
